# Design: "K01" — daily F&O stock screener skill

**Status: MVP built and deployed, 30 Aug 2026 — but turned OFF
(`K01_STRATEGY_ENABLED=false`) before its first live-market session
(31 Aug 2026), pending the rate-limit review below.** Named **"K01"** the
same day (renamed from its original `FnoScreener` working name, per user
request, "for future references"). Implementation lives in `traderBoy`'s
`K01/` package (paper-trading only, mirrors `IndexScalping`/
`CopperOptions`'s existing pattern), not in this repo — see that repo's
`README.md`/`NOTES.md` entries #39 (original build), #40 (the rename), and
#41 (the disable + rate-limit mitigations) for the code-level details.
This file stays the design record; update it if a later test shows a
threshold/weight below needs to change, per the repo's own style guide,
rather than silently changing the number.

**Real finding, not just a precaution — shared-infrastructure risk to
real-money trading.** K01 can never place a real order (asserted at
startup, no code path to real order placement exists) and has its own
fully independent position store — but it reuses the *same authenticated
Dhan connection and process* as the real Options/Futures strategies, so
its REST call volume (intraday watchlist polling) genuinely competes for
Dhan's undocumented rate limit (`traderBoy/NOTES.md` bug #5) against their
own exit-monitoring calls. This is the same mechanism as the stale-LTP
finding in `learnings/exit-mechanics.md` — K01 just adds real volume to
it. Mitigated (not yet re-enabled) via `K01_POLL_INTERVAL_SECONDS` 15→45s
and a new `K01_WATCHLIST_CAP=8` (was effectively 20). **Any future
paper-only strategy added to this shared process should go through the
same reasoning before its first live session** — "paper trading only"
guarantees no real order gets placed, but it does NOT by itself guarantee
zero impact on real trading when the strategy shares infrastructure
(connection, process, rate-limit budget) with one that does place real
orders.
...
**Done** — built as `traderBoy/K01/` (a full FastAPI strategy package
mirroring `IndexScalping`/`CopperOptions`'s existing pattern, not a
standalone script+SKILL.md as originally sketched here — it needs to run
continuously through market hours with live status updates, which a
one-shot script can't do). See that repo's `NOTES.md` entries #39/#40 for
the implementation details and the rename.

**Still open, phase 2:** Stage 2 (OI-buildup gating, needs Dhan's Option
Chain API) and Stage 0's VCP-detection bonus scoring — both deliberately
deferred out of the MVP, see the "MVP scope" note at the top of this file.
